...
class Conversations {
    ...
    async stream() {
        ...
        const decodeConversation = async (env) => {
            if (env.contentTopic === introTopic) {
                ...
                const msg = await Message_1.MessageV1.fromBytes(messageBytes);
    ...
    async streamAllMessages() {
        ...
        const str = await Stream_1.default.create(this.client, Array.from(topics.values()), decodeMessage, contentTopicUpdater);
    ...
    async newConversation(peerAddress, context) {
        ...
        if (contact instanceof crypto_1.PublicKeyBundle && !context?.conversationId) {
