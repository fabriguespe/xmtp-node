...
class NetworkKeyManager {
    ...
    async toEncryptedBytes(bundle, wallet) {
        ...
        const wPreKey = crypto_1.default.getRandomValues(new Uint8Array(32));
        ...
        const ciphertext = await (0, crypto_2.encrypt)(bytes, secret);
    ...
    async fromEncryptedBytes(wallet, bytes) {
        ...
        const secret = (0, utils_1.hexToBytes)(await wallet.signMessage(storageSigRequestText(eBundle.walletPreKey)));
        ...
        try {
            ...
            const decrypted = await (0, crypto_2.decrypt)(ciphertext, secret);
            ...
            const ciphertext = new Ciphertext_1.default(eBundle.ciphertext);
            const decrypted = await (0, crypto_2.decrypt)(ciphertext, newSecret);
            const [bundle, needsUpdate2] = getPrivateBundle(decrypted);
            return [bundle, needsUpdate || needsUpdate2];
        }
    }
...
function getPrivateBundle(bytes) {
    try {
        ...
        const b = (0, crypto_2.decodePrivateKeyBundle)(bytes);
        if (b instanceof crypto_2.PrivateKeyBundleV2) {
        ...
        return [new crypto_2.PrivateKeyBundleV1(b), true];
    }
}
