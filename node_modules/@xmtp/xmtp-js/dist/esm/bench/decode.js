...
const decodeV1 = () => {
    return MESSAGE_SIZES.map((size) => add(`decode and decrypt a ${size} byte v1 message`, async () => {
        const alice = await newLocalHostClient();
        const bob = await newPrivateKeyBundle();
        const message = randomBytes(size);
        const encodedMessage = await MessageV1.encode(alice.keystore, await alice.encodeContent(message), alice.publicKeyBundle, bob.getPublicKeyBundle(), new Date());
...
const decodeV2 = () => {
    return MESSAGE_SIZES.map((size) => add(`decode and decrypt a ${size} byte v2 message`, async () => {
        const alice = await newLocalHostClient();
        const bob = await newPrivateKeyBundle();
        const message = randomBytes(size);
        ...
        const convo = new ConversationV2(alice, invite.conversation?.topic ?? '', bob.identityKey.publicKey.walletSignatureAddress(), new Date(), undefined);
